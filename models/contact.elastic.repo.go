...
type ElasticContactSvcRepo interface {
	...
	Create(contact *ElasticContact) error
	Update(contact *ElasticContact) error
...
func (t *ElasticContactStruct) Create(contact *ElasticContact) error {
	...
	response, err := t.ElasticClient.Index(
		...
		t.ElasticClient.Index.WithDocumentID(contact.UUID),
	)
	if err != nil {
		return err
	}
...
func (t *ElasticContactStruct) Update(contact *ElasticContact) error {
	// In Elasticsearch, update is the same as index (upsert behavior)
	return t.Create(contact)
}

func (t *ElasticContactStruct) Delete(uuid string) error {
