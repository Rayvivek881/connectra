...
func OpenSearchContactFromRawData(contact *PgContact, company *PgCompany) *OpenSearchContact {
	serverTime := time.Now()
	osContact := &OpenSearchContact{
		UUID:        contact.UUID,
...
type OpenSearchContactSearchHit struct {
	Contact OpenSearchContact `json:"_source"`
	Cursor  []string          `json:"sort,omitempty"`
}

type OpenSearchContactSearchResponse struct {
	Hits struct {
		Hits []*OpenSearchContactSearchHit `json:"hits"`
	} `json:"hits"`
}

func (c *OpenSearchContact) SetClient(client *opensearch.Client) *OpenSearchContact {
	c.client = client
	return c
}
