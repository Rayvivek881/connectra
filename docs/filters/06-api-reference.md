...
**Rate Limiting**: Token bucket algorithm with configurable requests per minute (default: 60).
...
**Note**: Response includes both filterable fields (indexed in Elasticsearch) and response-only fields (PostgreSQL only). Fields like `facebook_url`, `twitter_url`, `website`, `work_direct_phone`, `home_phone`, `other_phone`, and `stage` are response-only and cannot be used in filter queries. Use `select_columns` to limit which fields are returned.
...
```json
{
  "error": "page size exceeds maximum limit",
  "success": false
}
```
...
```json
{
  "error": "elasticsearch error: status 400, body: ...",
  "success": false
}
```
...
```json
{
  "error": "elasticsearch error: connection refused",
  "success": false
}
```
